...
class Point:
    ...
    def distance(self, other_cell: Point) -> float:
        ...
        xsubtracted: float = other_cell.x - self.x
        ysubtracted: float = other_cell.y - self.y
        power: float = xsubtracted ** 2 + ysubtracted ** 2
        squared: float = sqrt(power)
        return squared
...
class Cell:
    ...
    def tick(self) -> None:
        """Makes changes to the current state of cell."""
...
class Model:
    ...
    def check_contacts(self) -> None:
        """Checks if any cells are touching."""
